...
void svp_init(void)
{
  svp = (void *) ((int8_t *)cart.rom + 0x200000);
...
void svp_write_dram(uint32_t address, uint32_t data)
{
...
uint32_t svp_read_cell_1(uint32_t address)
{
  address >>= 1;
  address = (address & 0x7001) | ((address & 0x3e) << 6) | ((address & 0xfc0) >> 5);
  return *(uint16_t *)(svp->dram + (address & 0x1fffe));
}

uint32_t svp_read_cell_2(uint32_t address)
{
  address >>= 1;
